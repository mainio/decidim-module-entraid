  ...
  module EntraID
    module Authentication
      class Authenticator
        ...
        def initialize(tenant, organization, oauth_hash, oauth_strategy)
          @tenant = tenant
        ...
        # be performed.
        def validate!
          raise ValidationError, "No data provided" unless user_attributes

          actual_attributes = user_attributes
          raise ValidationError, "No data provided" if actual_attributes.blank?

          data_blank = actual_attributes.all? { |_k, val| val.blank? }
          raise ValidationError, "Invalid data" if data_blank
        ...
        # registrations and authorization through EntraID is enabled in the
        # user's profile. This adds a new identity to an existing user unless
        # the identity is already bound to another user profile which would
        # happen e.g. in the following situation:
        #
        ...
        #   EntraID (which adds the authorization metadata information that
        #   might be required in order to perform actions).
        ...
        # new identity has been created to them. The possibly configured user
        ...
        attr_reader :organization, :tenant, :oauth_hash, :oauth_strategy
        ...
        def user_attributes
          @user_attributes ||= oauth_hash[:extra][:raw_info]
        end
        ...
        def metadata_collector
          @metadata_collector ||= tenant.metadata_collector_for(user_attributes, oauth_strategy)
        end
