      ...
      describe Engine do
        it "adds the correct routes" do
          expect(described_class.routes.recognize_path("/authorizations/new")).to eq(
            controller: "decidim/entraid/verification/authorizations",
            action: "new"
          )
        ...
        it "registers the verification workflow" do
          expect(Decidim::Verifications).to receive(
            :register_workflow
          ).with(:entraid_identity) do |&block|
            workflow = double
          ...
          run_initializer("decidim_entraid.verification_workflow")
        end

        describe "#load_seed" do
          before { create(:organization) }

          it "adds :entraid_identity to the organization's available authorizations" do
            described_class.load_seed

            org = Decidim::Organization.first
            expect(org.available_authorizations).to include("entraid_identity")
          end
        end
